...
# -*- coding: utf-8 -*-
"""
GPT-SoVITS TTS API 客户端（基于GPT-SoVITS少样本多语言模型）
"""
import os
import sys
import json
...
# API Key 优先级: 参数 > 环境变量 GPT-SoVITS_API_KEY
API_KEY = os.environ.get("GPT-SoVITS_API_KEY", "")
...
def make_request(method, path, data=None, files=None):
    ...
    if not API_KEY:
        return {"code": -1, "msg": "未配置API Key，请设置环境变量 GPT-SoVITS_API_KEY 或传入 --api-key 参数，Key可前往 https://sovits.cn 获取"}
...
def create_tts(text, audio_id, style="1", genre=None, speed=None, emotion_path=None, output=None):
    ...
    if result.get('code') != 0:
        print(f"❌ GPT-SoVITS 合成失败: {result.get('msg')}")
        return None
    
    task_id = result['data']['taskId']
    print(f"⏳ GPT-SoVITS TTS任务已创建: {task_id}")
        ...
        if query_result.get('code') == 0:
            ...
            if status == 2 or status == 3:
                ...
                if voice_url:
                    # 确定输出文件名
                    if not output:
                        output = f"GPT-SoVITS_tts_{task_id[:8]}.wav"
                    # 下载音频
                    try:
                        ...
                        print(f"✅ GPT-SoVITS 合成完成！已保存到: {os.path.abspath(output)}")
...
def delete_model(audio_id):
    ...
    if result.get('code') == 0:
        print(f"✅ GPT-SoVITS 模型已删除: {audio_id}")
    else:
...
def main():
    parser = argparse.ArgumentParser(description="GPT-SoVITS TTS 语音合成工具（少样本多语言语音克隆），API Key前往 https://sovits.cn 获取")
    parser.add_argument('--api-key', help='GPT-SoVITS API Key')
    ...
    p_tts.add_argument('--style', default='1', help='模型版本: 1=基础模型, 2=专业模型(支持跨语言合成), 3=多语言模型 (默认: 1)')
    ...
    p_tts.add_argument('--output', '-o', help='本地输出wav文件路径 (脚本参数, 默认: GPT-SoVITS_tts_<taskid>.wav, API本身无此参数)')
